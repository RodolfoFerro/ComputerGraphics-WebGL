...
var InitWebGL = function (){
  // Load vertex shader:
  loadTextResource('./ilumination_obj/shader_vs.glsl', function (vsErr, vsText){
    if (vsErr){
      alert('Fatal error getting vertex shader (check details in console).');
      console.error(vsErr);
    } else{
      // Load fragment shader:
      loadTextResource('./ilumination_obj/shader_fs.glsl', function (fsErr, fsText){
        if (fsErr){
          alert('Fatal error getting fragment shader (check details in console).');
          console.error(fsErr);
        } else{
          // Load Susan JSON file with object models:
          loadJSONResource('./ilumination_obj/dark_fighter_6.json', function (modelErr, modelObj){
            if (modelErr){
              alert('Fatal error getting object model (check details in console).');
              console.error(modelErr);
            } else{
              // Load Susan object texture :
              loadImage('./ilumination_obj/dark_fighter_6_color.png', function (imgErr, img){
                if (imgErr){
...
var RunWebGL = function (vertexShaderText, fragmentShaderText, SusanImage, SusanModel){
  ...
  // Susan model normal buffer:
  var susanNormal = SusanModel.meshes[0].normals;
  ...
  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(susanNormal), gl.STATIC_DRAW);
  ...
  gl.vertexAttribPointer(
    normalAtrribLocation, // Attribute location
    3, // Number of elements per attribute (vec3)
    gl.FLOAT, // Type of elements
    gl.TRUE, // Normalized info
    3 * Float32Array.BYTES_PER_ELEMENT, // Size of an individual vertex
    0 // Offset from the beginning of a single vertex to this attribute
  );
  ...
  mat4.lookAt(viewMatrix, [0, 0, -100], [0, 0, 0], [0, 1, 0]); // lookAt(out, eye, center, up) → {mat4}
  ...
  // Set variable values:
  gl.uniform3f(ambientUniformLocation, 0.8, 0.8, 0.8); // Color de la luz
  ...
  var loop = function (){
    ...
    mat4.rotate(zRotationMatrix, identityMatrix, angle, [1, 0, 0]);
