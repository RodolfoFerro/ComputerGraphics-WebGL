...
LightMapDemoScene.prototype.Load = function (cb) {
	...
	async.parallel({
		Models: function (callback) {
			async.map({
				RoomModel: './scene_movement/Room.json'
			}, loadJSONResource, callback);
		},
		ShaderCode: function (callback) {
			async.map({
				'NoShadow_VSText': './scene_movement/shaders/NoShadow_vs.glsl',
				'NoShadow_FSText': './scene_movement/shaders/NoShadow_fs.glsl',
				'Shadow_VSText': './scene_movement/shaders/Shadow_vs.glsl',
				'Shadow_FSText': './scene_movement/shaders/Shadow_fs.glsl',
				'ShadowMapGen_VSText': './scene_movement/shaders/ShadowMapGen_vs.glsl',
				'ShadowMapGen_FSText': './scene_movement/shaders/ShadowMapGen_fs.glsl'
    ...
    // Check if models loaded successfuly:
		if (!me.MonkeyMesh) {
    ...
    // Create uniforms and attributes (from shaders):
		...
		me.camera = new Camera(
			vec3.fromValues(3.3, 3, 1.85),
			vec3.fromValues(-0.5, -1.0, 1.85),
			vec3.fromValues(0, 0, 1)
		);
		...
		me.shadowClipNearFar = vec2.fromValues(0.05, 30.0);
		mat4.perspective(
			me.shadowMapProj,
...
LightMapDemoScene.prototype._Update = function (dt) {
	...
	this.LightMesh.world[14] = xDisplacement;
